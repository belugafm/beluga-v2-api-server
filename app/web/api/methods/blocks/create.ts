...
export const facts: MethodFacts = {
    url: MethodIdentifiers.CreateBlock,
    ...
    async (args, errors, auth_user) => {
        try {
            ...
            if (error instanceof WebApiRuntimeError) {
                ...
                } else if (error.code === ModelErrorCodes.CannotBlockSelf) {
                    raise(errors.cannot_block_self, error)
                } else if (error.code === ModelErrorCodes.AlreadyBlocked) {
                    raise(errors.already_blocked, error)
                } else {
