...
export const expected_error_specs = define_expected_errors(
    ...
    argument_specs,
    {
        ...
        cannot_unmute_self: {
            ...
            code: "cannot_unmute_self",
        },
        already_unmuted: {
            description: ["すでにミュートを解除しています"],
...
export const facts: MethodFacts = {
    url: MethodIdentifiers.DestroyMute,
    ...
    async (args, errors, auth_user) => {
        try {
            ...
            if (error instanceof WebApiRuntimeError) {
                ...
                } else if (error.code === ModelErrorCodes.CannotUnmuteSelf) {
                    raise(errors.cannot_unmute_self, error)
                } else if (error.code === ModelErrorCodes.AlreadyUnmuted) {
                    raise(errors.already_unmuted, error)
                } else {
